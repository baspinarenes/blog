---
title: "Kuralları Yıkmayın: ESLint (1) - Temel Bilgiler"
description: "Sürekli hata fırlatan ancak her şeye rağmen asla vazgeçemediğimiz ESLint'in ne işe yaradığını görelim."
pubDate: 2022-04-15
slug: kurallari-yikmayin-eslint-1
draft: false
heroImage: "/src/assets/articles/frontend/kurallari-yikmayin-eslint/1-temel-bilgiler/hero-image.png"
tags:
  - linter
---
...
edindiğim deneyimleri bir yazıya dökeceğim.

<MessageBox type="info">
  Az sonra kitabi olmasa da bir tanımla karşılaşacaksınız. Tanımları hiç sevmem o yüzden çok üzerinde durmayacağım. Çok
  üzerinde durmadan arkalara doğru ilerleyelim. Bizim işimiz tanım ezberlemek değil, işin özüne dair fikir edinmek.
</MessageBox>
...
aşağıdaki gibi `for` içinde değişkeni deklare etmeyi unutursanız
ya da 100'den geriye saymayı amaçladığınız bir kodda değişkenin her döngüde artmasını söylerseniz döngü yönünün
...
![](/src/assets/articles/frontend/kurallari-yikmayin-eslint/1-temel-bilgiler/002.webp)
...
? Where does your code run? >
  ✔ Node
? What format do you want your config file to be in?What format do you want your config file to be in?What format do you want your config file to be in?
  ✔ JavaScript
```
...
_extends_ kısmı, kural setlerini oluşturan ekibin tavsiye
...
sayede sadece özelleştirmek istediğimiz kuralları _rules_ altında
belirtiriz. Ancak şuan için başka hatalarla uğraşmadan senaryomuza odaklanmak
adına ilgili satırı silerek diğer tüm kuralları kapatacağız.

<MessageBox type="info">
  Blog yazısı içerisinde altı noktalı metinlere denk gelirseniz üzerinde durun. Belirttiğim kısımların kod bloğundaki
  yerini gösterir.
</MessageBox>
...
<MessageBox type="info">
  Kuralların tam olarak nasıl çalıştıklarına serinin diğer yazılarında değineceğiz. Önce uygulanışını görelim ki
  anlatırken kafamızda eşleşsin.
</MessageBox>
...
![](/src/assets/articles/frontend/kurallari-yikmayin-eslint/1-temel-bilgiler/003.jpeg)
...
Halihazırda bahsetmiş olduğumuz _ESLint paketi_, terminalden çalıştırdığımız
komutla kodumuzu analiz eden ve uymayanları konsoldan bize bildirir. _VSCode
ESLint eklentisi_ ise arkaplanda ESLint paketini kullanır ve hataları hiçbir komut
çalıştırmadan, kodu yazarken editörde görmemizi sağlar.

Hemen indirip deneyelim.

![](/src/assets/articles/frontend/kurallari-yikmayin-eslint/1-temel-bilgiler/004.png)

İndirdikten sonra `src/sum.js` dosyamızı en baştaki hale getirelim ve eklentinin
yaptığı sihre odaklanalım.

![](/src/assets/articles/frontend/kurallari-yikmayin-eslint/1-temel-bilgiler/005.gif)
...
<MessageBox type="info">
  Her kaydedişte düzeltme yapmasını istemeyeceğimiz durumlar olabilir. Aşağıdaki değişikliği atlayıp VSCode'a sağ
  tıkladığımızda çıkan menüden 'Format Document' seçeneğini kullanabilirsiniz.
</MessageBox>
...
```javascript title="src/freedom.js"
/* eslint-disable */
...
canlı görebiliriz. Aynı zamanda otomatik formatlamayı dosya kaydedildiğinde veya
tuş kombinasyonlarına basıldığında tetikleyebiliriz.
