...
description: "Koddaki hataları en aza indirmek için ESLint'e göz atalım."
pubDate: 2023-04-30
...
Bir zamanlar front-end'çilerin bir takım problemleri vardı. Bazen kodlardaki hataları görmek zordu, sinsi bug'ları farkedebilmek ise günlerimizi yiyordu. Ekip projelerindeki kod standardını sağlamak zordu, bir yazılımcının `if` dediğine diğer `ternary` diyordu. Bu sorunların çözümü insan faktörüne bırakıldığında işin içinde çıkılamıyordu, dolayısıyla bağımsız bir göze yani yazılıma ihtiyaç vardı.
